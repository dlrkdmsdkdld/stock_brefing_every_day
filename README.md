...
| 추천 종목 (`recommend.py`) | 후보 5 | 1 | 1,354 |
...
오늘 들여다볼 만한 종목 하나를 고릅니다. 두 단계로 나눠 토큰을 아낍니다.

1. **데이터로 후보 좁히기** (토큰 0) — 거래량 배수, 등락폭, 뉴스 화제성, 볼린저 이탈,
   RSI 극단, 52주 고저 근접을 점수로 합산해 상위 5개를 고릅니다.
   뉴스 화제성은 오늘 잡힌 기사 수인데, 국내는 네이버·구글을 함께 훑어 기사가 원래 많으므로
   절대 건수 대신 **같은 시장 안에서의 상대 순위**로 환산합니다.
2. **후보 5개만 모델에 전달** — 기사 본문은 보내지 않고 이미 만들어 둔 **요약**만 보냅니다.
   그래서 요청 1번, 실측 **입력 994 / 출력 360 토큰**으로 끝납니다.

결과는 고른 종목·한 줄 이유·근거·유의점과 나머지 후보 코멘트로 나옵니다.
모델이 후보 밖 종목을 고르면 저장하지 않고 실패로 처리합니다.
